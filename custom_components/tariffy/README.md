...
- **Verbrauch (Bisher)** = `sum(jetzt) − sum(vertragsbeginn)` = **1.481,17 kWh**. Der Offset zum Vertragsbeginn wird einmalig per LTS-Abfrage ermittelt und danach gecacht. Tariffy liest dabei zusätzlich den rohen Zählerstand (`state`) mit: ist der aktuelle Stand nicht unter den Wert bei Vertragsbeginn gefallen (kein echter Reset), wird die einfachere State-Differenz statt der `sum`-Statistik verwendet — das macht die Berechnung robust gegenüber seltenen Anomalien direkt in der `sum`-Statistik selbst (beobachtet z. B. nach einem HA-Neustart: die Statistik sprang plötzlich auf einen unplausiblen Wert, während der Zählerstand durchgehend korrekt blieb). Reicht die Statistik-Historie deines Verbrauchssensors dennoch nicht bis zum Vertragsbeginn zurück (z. B. weil der Sensor zwischenzeitlich umbenannt oder neu angelegt wurde), kannst du optional einen **manuellen "Zählerstand bei Vertragsbeginn"** eintragen — dieser hat dann Vorrang vor beiden automatischen Methoden.
